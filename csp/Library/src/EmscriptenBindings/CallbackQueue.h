...
namespace csp
{
// Setup our proxy queue to send callbacks to the main thread
...
template <typename... T>
struct CallbackDataEmbind
{
    emscripten::val& Callback; //Must be statically pinned, you can't even touch vals on the wrong thread
    std::tuple<T...> Args;
};

template <typename... T>
static void Emscripten_CallbackWrapperEmbind(void* InData)
{
    auto* Data = static_cast<CallbackDataEmbind<T...>*>(InData);
    std::apply([&](T&... Args) { Data->Callback(Args...); }, Data->Args);
}

//MUST take a pinned callback
template <typename... T>
static void Emscripten_CallbackOnThreadEmbind(emscripten::val& Callback, T... Args)
{
    bool OnMainThread = pthread_equal(pthread_self(), emscripten_main_runtime_thread_id());
    if (OnMainThread)
    {
        std::cout << "Calling on main thread" << std::endl;
        Callback(Args...);
    }
    else
    {
        std::cout << "Passing into proxy queue" << std::endl;
        CallbackDataEmbind<T...> Data { Callback, std::tuple<T...> { std::move(Args)... } };
        emscripten_proxy_sync(ProxyQueue, MainThread, Emscripten_CallbackWrapperEmbind<T...>, static_cast<void*>(&Data));
    }
}
