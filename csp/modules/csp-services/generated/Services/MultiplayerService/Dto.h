...
class NamedFunction;
class ObjectMessageDocumentDto;
class ObjectMessageDocumentDtoDataPage;
...
public:
	...
	/// <summary>
	/// User unique identifier of the individual sending this event
...
/// <summary>
/// Represents a multiplayer object message document that can be used to store and transfer object messages in a database.
/// </summary>
class ObjectMessageDocumentDto : public csp::services::DtoBase
{
public:
	ObjectMessageDocumentDto();
	virtual ~ObjectMessageDocumentDto();
	...
	/// <summary>
	/// The user unique identifier for the current owner
	/// </summary>
	...
	/// but for authorization purposes, the owner of the object is actually Magnopus.Service.Multiplayer.Dtos.ObjectMessageDocumentDto.OwnerUserId.
	/// If set, this user will override Magnopus.Service.Multiplayer.Dtos.ObjectMessageDocumentDto.OwnerUserId for lookups of the assigned scopes and
	/// groups to propagate messages.
	/// </summary>
	...
	/// <summary>
	/// The reference type of the scope, "GroupId" for example
	/// </summary>
	...
	std::shared_ptr<ObjectMessageDto> GetObjectMessage() const;
	void SetObjectMessage(const std::shared_ptr<ObjectMessageDto>& Value);
	bool HasObjectMessage() const;

	/// <summary>
	/// Standard unit based position in flat euclidean space
	/// </summary>
	const std::vector<float>& GetEuclideanPosition() const;
	void SetEuclideanPosition(const std::vector<float>& Value);
	bool HasEuclideanPosition() const;

	/// <summary>
	/// The root parent (object message document ID) of this object.
	/// </summary>
	utility::string_t GetRootParentId() const;
	void SetRootParentId(const utility::string_t& Value);
	bool HasRootParentId() const;
	...
	/// <summary>
	/// Sets flag if this is the object currently controlled by a player
	/// </summary>
...
protected:
	...
	std::optional<std::shared_ptr<ObjectMessageDto>> m_ObjectMessage;
	std::optional<std::vector<float>> m_EuclideanPosition;
	std::optional<utility::string_t> m_RootParentId;
	std::optional<utility::string_t> m_DuplicatedFrom;
	std::optional<bool> m_AoiCenter;
};

class ObjectMessageDocumentDtoDataPage : public csp::services::DtoBase
{
public:
	ObjectMessageDocumentDtoDataPage();
	virtual ~ObjectMessageDocumentDtoDataPage();

	utility::string_t ToJson() const override;
	void FromJson(const utility::string_t& Json) override;


	const std::vector<std::shared_ptr<ObjectMessageDocumentDto>>& GetItems() const;
	void SetItems(const std::vector<std::shared_ptr<ObjectMessageDocumentDto>>& Value);
	bool HasItems() const;
...
protected:
	std::optional<std::vector<std::shared_ptr<ObjectMessageDocumentDto>>> m_Items;
	std::optional<int32_t> m_Skip;
	std::optional<int32_t> m_Limit;
	std::optional<int32_t> m_ItemCount;
	std::optional<int64_t> m_ItemTotalCount;
...
public:
	...
	/// <summary>
	/// The user that is the current owner of the object
	/// Note: `null` will use the current user
	/// </summary>
	utility::string_t GetOwnerUserId() const;
	void SetOwnerUserId(const utility::string_t& Value);
...
public:
	...
	/// <summary>
	/// Request to transfer ownership of this object
	/// Note: `null` will use the current user
	/// </summary>
	utility::string_t GetOwnerUserId() const;
	void SetOwnerUserId(const utility::string_t& Value);
...
public:
	...
	/// <summary>
	/// Client Id of the currently leader
	/// </summary>
	int32_t GetLeaderClientId() const;
	void SetLeaderClientId(int32_t Value);
	bool HasLeaderClientId() const;
